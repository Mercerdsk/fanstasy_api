...
pub async fn req_res_log(method:&str,_reqid:f64,logroot:&str,data:String) {
    let api_url = "http://192.168.10.155:12207/gelf"; // Replace with your API URL
...
pub async fn error_log(method:&str,_reqid:f64,logroot:&str,data:String) {
    let api_url = "http://192.168.10.155:12207/gelf"; // Replace with your API URL
    let client = Client::new();
    let mut json_body = HashMap::new();
    ...
    // json_body.insert("_ip",client_ip);
    json_body.insert("_reqid",_reqid.to_string());
    json_body.insert("facility","1".to_string());
    json_body.insert("_logroot",logroot.to_string());
    json_body.insert("_message","logroot".to_string());
    json_body.insert("full_message",data);
